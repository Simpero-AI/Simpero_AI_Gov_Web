...
export const DEALS_DASHBOARD_STATS_QUERY_KEY = [
  "deals",
  "dashboardStats",
] as const;
export const dealQueryKey = (dealId: string) =>
  ["deals", "get", dealId] as const;
export const dealStatusQueryKey = (dealId: string) =>
  ["deals", "status", dealId] as const;
...
export async function fetchDashboardStats(): Promise<DashboardStatsPayload> {
  ...
  if (!res.ok)
    throw new Error(`GET /deals/dashboard-stats failed: ${res.status}`);
...
export async function fetchDeal(
  dealId: string
): Promise<DealWithLatestMemo | null> {
...
export async function fetchDealStatus(
  dealId: string
): Promise<DealStatusPayload> {
  const res = await apiFetch(`/api/deals/${dealId}/status`);
  if (!res.ok)
    throw new Error(`GET /deals/${dealId}/status failed: ${res.status}`);
...
export async function startDealAnalysis(
  ...
  if (!res.ok) {
    ...
    throw new AnalysisApiError(
      res.status,
      errBody.detail ?? `POST /deals/${dealId}/analysis failed: ${res.status}`
    );
  }
  return (await res.json()) as DealStatusPayload;
}
